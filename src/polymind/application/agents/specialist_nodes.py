...
def asr_node(state: PolyMindState) -> PolyMindState:
    ...
    try:
        ...
        asr = ASRWrapper()
        # ASR is sync, wrap in blocking call
        import asyncio

        result = asyncio.get_event_loop().run_until_complete(
            asr.process(audio_path)
        )
...
def vqa_node(state: PolyMindState) -> PolyMindState:
    ...
    try:
        ...
        result = asyncio.get_event_loop().run_until_complete(
            vqa.process(image_path, question=question)
        )
...
def docqa_node(state: PolyMindState) -> PolyMindState:
    ...
    try:
        ...
        result = asyncio.get_event_loop().run_until_complete(
            docqa.process(file_path, question=question)
        )
...
def tableqa_node(state: PolyMindState) -> PolyMindState:
    ...
    try:
        ...
        result = asyncio.get_event_loop().run_until_complete(
            tableqa.process(file_path, question=question)
        )
