...
from polymind.infrastructure.rag.chunker import RecursiveChunker
...
class IngestionPipeline:
    ...
    def __init__(self, embedder: Embedder, collection: str = "polymind") -> None:
    ...
    async def ingest_file(
        ...
        logger.info("ingestion.chunks.created", count=len(doc_chunks))
        return doc_chunks

    def _extract_text(self, path: Path, file_type: str) -> str:
        """Extract text content from a file based on its type."""
        if file_type in ("txt", "md", "py", "json"):
